...
class PointSegmentationDataset(Dataset):
    ...
    Points are sampled deterministically based on image path to ensure the same image
    always gets the same points (mimicking expert annotations that are fixed).
    """
    ...
    def __getitem__(self, idx):
        ...
        # Use deterministic seeding based on image path to ensure same image always gets same points
        # This mimics expert annotations that are fixed per image
        image_path_str = self.image_paths[idx]
        # Create a hash-based seed from the image path for reproducibility
        seed = hash(image_path_str) % (2**31)  # Ensure positive 32-bit integer
        ...
        return (
            image,
            mask,
            torch.tensor(all_points, dtype=torch.long),
...
def make_dataloaders(
    ...
    return (
        torch.utils.data.DataLoader(
            train_ds, batch_size=batch_size, shuffle=True, collate_fn=collate_points_fn
        ),
        torch.utils.data.DataLoader(
            test_ds, batch_size=batch_size, shuffle=False, collate_fn=collate_points_fn
        ),
    )
